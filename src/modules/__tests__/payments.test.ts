...
describe('Payments', () => {
  ...
  it('creates a Stripe checkout session through the provider-scoped runtime route', async () => {
    ...
    expect(url).toBe('http://localhost:7130/api/payments/stripe/test/checkout-sessions');
  ...
  it('creates a Stripe customer portal session through the provider-scoped runtime route', async () => {
    ...
    expect(result.error).toBeNull();
    expect(result.data?.customerPortalSession.url).toBe('https://billing.stripe.com/p/session_123');
    expect(fetchFn).toHaveBeenCalledOnce();

    const [url, init] = fetchFn.mock.calls[0] as [string, RequestInit];
    expect(url).toBe('http://localhost:7130/api/payments/stripe/test/customer-portal-sessions');
    expect(init.method).toBe('POST');
    expect(JSON.parse(init.body as string)).toMatchObject({
      subject: { type: 'team', id: 'team_123' },
    });
  ...
  it('creates a Razorpay order and returns Checkout.js options', async () => {
    const fetchFn = vi.fn().mockResolvedValue(jsonRes(201, razorpayOrderResponse()));
    const payments = new Payments(makeHttp(fetchFn));
  ...
  it('verifies a Razorpay order payment signature', async () => {
    ...
    const [url, init] = fetchFn.mock.calls[0] as [string, RequestInit];
    expect(url).toBe('http://localhost:7130/api/payments/razorpay/test/orders/verify');
  ...
  it.each([
    ...
    },
    {
      ...
      call: (payments: Payments) => payments.razorpay.cancelSubscription('test', 'sub_123'),
    },
    {
      method: 'pauseSubscription',
      fallbackMessage: 'Razorpay subscription pause failed',
      call: (payments: Payments) => payments.razorpay.pauseSubscription('test', 'sub_123'),
    },
    {
